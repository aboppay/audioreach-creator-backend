/*
 * Copyright (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
 * SPDX-License-Identifier: BSD-3-Clause
 */

import type {DataSource, QueryRunner} from 'typeorm';
import {CHANGE_OPERATION, CHANGE_STATUS, SOURCE} from '@arc/core';
import {
  SESSION_MODE,
  SESSION_STATUS,
} from '../../../src/persistence-typeorm-sqllite/entity-schema/edit-session/project-session.schema.js';
import {
  setupIntegrationTest,
  teardownIntegrationTest,
  setupEachTest,
  getTestDataSource,
  getTestRepository,
} from '../helpers/test-database-setup.js';
import {EditActionsQueryService} from '../../../src/persistence-typeorm-sqllite/queries/edit-session/edit-actions-query-service.js';
import {ContainerOverlayFetcher} from '../../../src/persistence-typeorm-sqllite/fetchers/container-overlay-fetcher.js';
import {ContainerPropertyDataFetcher} from '../../../src/persistence-typeorm-sqllite/fetchers/container-property-data-fetcher.js';
import {ENTITY_NAMES} from '../../../src/persistence-typeorm-sqllite/entity-schema/entity-table-names.js';
import {ProjectSchema} from '../../../src/persistence-typeorm-sqllite/entity-schema/project-data/project.schema.js';
import {ArcDbFileSchema} from '../../../src/persistence-typeorm-sqllite/entity-schema/project-data/arc-db-file.schema.js';
import {ProjectSessionSchema} from '../../../src/persistence-typeorm-sqllite/entity-schema/edit-session/project-session.schema.js';
import {
  describe,
  it,
  expect,
  beforeAll,
  afterAll,
  beforeEach,
  afterEach,
} from '@jest/globals';

const FILE_ID = 100;
const CONTAINER_ID = 42;

async function seedProjectAndFile(ds: DataSource) {
  await getTestRepository(ProjectSchema).save({
    systemId: 1,
    name: 'P',
    description: '',
    type: 'Offline',
  });
  await getTestRepository(ArcDbFileSchema).save({
    systemId: FILE_ID,
    projectSystemId: 1,
    fileName: 'f.acdb',
    description: '',
    metadata: '{}',
    isTarget: true,
    lastReservedId: 0,
  });
}

async function seedSession(ds: DataSource): Promise<number> {
  const row = await getTestRepository(ProjectSessionSchema).save({
    fileSystemId: FILE_ID,
    userId: 'u',
    clientId: 'c',
    sessionMode: SESSION_MODE.Designer,
    status: SESSION_STATUS.Active,
    endedAt: null,
  });
  return row.sessionId;
}

async function seedContainer(
  ds: DataSource,
  opts: {
    systemId: number;
    fileSystemId: number;
    containerTypeSystemId?: number;
  },
) {
  await ds.query(
    `INSERT INTO containers (system_id, container_id, container_type_system_id, file_system_id) VALUES (?, 1, ?, ?)`,
    [opts.systemId, opts.containerTypeSystemId ?? 5, opts.fileSystemId],
  );
}

async function seedEditAction(
  ds: DataSource,
  opts: {
    sessionId: number;
    aggregateId: number;
    targetSystemId: number;
    targetTable: string;
    operation: string;
    newValue: string;
    fieldPath?: string | null;
  },
) {
  await ds.query(
    `INSERT INTO edit_actions (session_id, aggregate_id, target_system_id, target_table, operation, field_path, new_value, source, change_status, group_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)`,
    [
      opts.sessionId,
      opts.aggregateId,
      opts.targetSystemId,
      opts.targetTable,
      opts.operation,
      opts.fieldPath ?? null,
      opts.newValue,
      SOURCE.Manual,
      CHANGE_STATUS.Staged,
    ],
  );
}

describe('ContainerOverlayFetcher (integration)', () => {
  let ds: DataSource;
  let qr: QueryRunner;
  let fetcher: ContainerOverlayFetcher;

  beforeAll(async () => {
    await setupIntegrationTest();
  });
  afterAll(async () => {
    await teardownIntegrationTest();
  });
  beforeEach(async () => {
    await setupEachTest();
    ds = getTestDataSource();
    await seedProjectAndFile(ds);
    qr = ds.createQueryRunner();
    await qr.connect();
    const editActionsQs = new EditActionsQueryService(qr.manager);
    fetcher = new ContainerOverlayFetcher(
      qr.manager,
      editActionsQs,
      new ContainerPropertyDataFetcher(qr.manager, editActionsQs),
    );
  });
  afterEach(async () => {
    await qr.release();
  });

  it('returns null when no base row and sessionId is null', async () => {
    expect(await fetcher.fetchOne(CONTAINER_ID, FILE_ID, null)).toBeNull();
  });

  it('returns base container when sessionId is null', async () => {
    await seedContainer(ds, {
      systemId: CONTAINER_ID,
      fileSystemId: FILE_ID,
      containerTypeSystemId: 5,
    });
    const result = await fetcher.fetchOne(CONTAINER_ID, FILE_ID, null);
    expect(result).not.toBeNull();
    expect(result!.containerTypeSystemId).toBe(5);
    expect(result!.properties).toHaveLength(0);
  });

  it('returns CREATE-staged container even with no base row — the auto-create case', async () => {
    const sessionId = await seedSession(ds);
    await seedEditAction(ds, {
      sessionId,
      aggregateId: CONTAINER_ID,
      targetSystemId: CONTAINER_ID,
      targetTable: ENTITY_NAMES.Container,
      operation: CHANGE_OPERATION.Create,
      newValue: JSON.stringify({
        naturalId: 1,
        containerTypeSystemId: 5,
        fileSystemId: FILE_ID,
      }),
    });
    const result = await fetcher.fetchOne(CONTAINER_ID, FILE_ID, sessionId);
    expect(result).not.toBeNull();
    expect(result!.systemId).toBe(CONTAINER_ID);
    expect(result!.containerTypeSystemId).toBe(5);
  });

  it('applies UPDATE overlay to containerTypeSystemId', async () => {
    await seedContainer(ds, {
      systemId: CONTAINER_ID,
      fileSystemId: FILE_ID,
      containerTypeSystemId: 1,
    });
    const sessionId = await seedSession(ds);
    await seedEditAction(ds, {
      sessionId,
      aggregateId: CONTAINER_ID,
      targetSystemId: CONTAINER_ID,
      targetTable: ENTITY_NAMES.Container,
      operation: CHANGE_OPERATION.Update,
      fieldPath: 'containerTypeSystemId',
      newValue: '9',
    });
    const result = await fetcher.fetchOne(CONTAINER_ID, FILE_ID, sessionId);
    expect(result!.containerTypeSystemId).toBe(9);
  });

  it('tombstones DELETE-staged container', async () => {
    await seedContainer(ds, {
      systemId: CONTAINER_ID,
      fileSystemId: FILE_ID,
      containerTypeSystemId: 5,
    });
    const sessionId = await seedSession(ds);
    await seedEditAction(ds, {
      sessionId,
      aggregateId: CONTAINER_ID,
      targetSystemId: CONTAINER_ID,
      targetTable: ENTITY_NAMES.Container,
      operation: CHANGE_OPERATION.Delete,
      newValue: '{}',
    });
    expect(await fetcher.fetchOne(CONTAINER_ID, FILE_ID, sessionId)).toBeNull();
  });

  it('includes CREATE-staged container property', async () => {
    await seedContainer(ds, {
      systemId: CONTAINER_ID,
      fileSystemId: FILE_ID,
      containerTypeSystemId: 5,
    });
    const sessionId = await seedSession(ds);
    const propSystemId = 300;
    await seedEditAction(ds, {
      sessionId,
      aggregateId: CONTAINER_ID,
      targetSystemId: propSystemId,
      targetTable: ENTITY_NAMES.ContainerPropertyData,
      operation: CHANGE_OPERATION.Create,
      newValue: JSON.stringify({
        containerSystemId: CONTAINER_ID,
        propertySystemId: 7,
        payload: null,
      }),
    });
    const result = await fetcher.fetchOne(CONTAINER_ID, FILE_ID, sessionId);
    expect(
      result!.properties.find(p => p.systemId === propSystemId),
    ).toBeDefined();
  });
});
